...
bool AShooterCharacter::TimeSync_Validate(FDateTime DateTime)
{
	return true;
}

void AShooterCharacter::TimeSync_Implementation(FDateTime DateTime)
{
	float RTT = GetPlayerRTT();
	FDateTime Now = FDateTime::Now();
	// 클라이언트 시간.
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Client Time %lld"), DateTime.GetTicks());
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Rtt to tick %lld"), FTimespan::FromMilliseconds(RTT).GetTicks());
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Now to tick %lld"), Now.GetTicks());
	FDateTime PredictClientNow = DateTime + FTimespan::FromMilliseconds(RTT);
	ClientSyncTimespan = PredictClientNow - Now;
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] ClientSyncTimespan %lld"), ClientSyncTimespan.GetTicks());
}

float AShooterCharacter::GetPlayerRTT()
{
	APlayerState* PlayerState = GetPlayerState();
	if (PlayerState == nullptr)
	{
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Failed GetPlayerState"));
		return -1;
}

	return PlayerState->ExactPing / 2.0f;
}
...
void AShooterCharacter::BeginPlay() 
{
	Super::BeginPlay();
#if UE_SERVER
	CharacterTransformWithTimeTimer = FDateTime::Now();
#else
	GetWorldTimerManager().SetTimer(SyncTimeWithServerHandle, this, &AShooterCharacter::SyncTimerWithServer, 2.0f, true, 1.0f);
	auto animation_script_instance = GetMesh()->AnimScriptInstance;
	if (animation_script_instance == nullptr) {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Not has animation script instance"));
		return;
	}
	else {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Has animation script instance"));
	}


	
	if (animation_script_instance->IsInBlueprint()) {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Player character use Blueprint Animation"));
	}
	else {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Player character not use Blueprint Animation"));
	}

	auto montage_instance = animation_script_instance->GetActiveMontageInstance();
	if (montage_instance != nullptr) {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Montage Instance"));
	}
	else {
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Not has Montage Instance"));
	}

	//TArray<FTransform> a = GetMesh()->GetBoneSpaceTransforms();
	//GetMesh()->BoneSpaceTransforms
#endif
}


void AShooterCharacter::UpdatePreviousTransform()
{
	PreviousTransform = GetMesh()->GetBoneSpaceTransforms();
	ComponentTransform = GetMesh()->GetComponentSpaceTransforms();
	GetMesh()->GetAnimInstance()->SnapshotPose(MySnapshot);
}

void AShooterCharacter::SetBoneTransform(FPoseSnapshot SnapShot_)
{
	//GetMesh()->BoneSpaceTransforms = BoneTransform_;
	ChangeSnapShot = true;
	MySnapshot = SnapShot_;
	//FBlendedHeapCurve A;
	//GetMesh()->PerformAnimationProcessing(GetMesh()->SkeletalMesh, nullptr, true, ComponentTransform_ ,BoneTransform_, GetMesh()->RootBoneTranslation, A);
	
}


void AShooterCharacter::SyncTimerWithServer()
{
	TimeSync(FDateTime::Now());
}

#if UE_SERVER
void AShooterCharacter::UpdateTransformWithTimer()
{
	const FTransform &TransformData = GetTransform();
	FVector Location = TransformData.GetLocation();
	FVector Rotation = TransformData.GetRotation().Vector();
	FDateTime Now = FDateTime::Now();

	FTransformWithTime CurrentTransfromWithTime;
	CurrentTransfromWithTime.Initialized = true;
	CurrentTransfromWithTime.Location = Location;
	CurrentTransfromWithTime.Rotation = Rotation;
	CurrentTransfromWithTime.DateTime = Now;

	// 순서가 일치하는지 확인해야함.
	TransformWithTime.Add(CurrentTransfromWithTime);
	if (TransformWithTime.Num() > 128) 
	{
		TransformWithTime.RemoveAt(0);
	}
}

FTransformWithTime AShooterCharacter::GetTransformAtTime(FDateTime DateTime, FTimespan Other_ClientSyncTimespan)
{
	float RTT = GetPlayerRTT();
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Other Client Rtt %f"), RTT);
	FDateTime SyncDateTime = DateTime - Other_ClientSyncTimespan;// +FTimespan::FromMilliseconds(RTT);
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] SyncDateTime %lld"), SyncDateTime.GetTicks());
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Other_ClientSyncTimespan %lld"), Other_ClientSyncTimespan.GetTicks());

	int Num = TransformWithTime.Num();
	if (Num == 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Not has Transfrom data"));
		return FTransformWithTime();
	}
	...
	while (Loop)
	{
		UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Begin %d, End %d"), Begin, End);
		if (Begin >= 127 && End >= 127) 
		{
			FTransformWithTime CurrentTransfromWithTime;
			const FTransform& TransformData = GetTransform();
			CurrentTransfromWithTime.Location = TransformData.GetLocation();
			CurrentTransfromWithTime.Rotation = TransformData.GetRotation().Vector();
			CurrentTransfromWithTime.Initialized = true;
			return CurrentTransfromWithTime;
		}

		int Mid = (Begin + End) / 2;
		if (SyncDateTime > TransformWithTime[Mid].DateTime)
		{
			// 이분 탐색의 응용.
			if (SyncDateTime <= TransformWithTime[Mid + 1].DateTime)
			{
				Begin = Mid;
				End = Mid + 1;
				UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] DateTime <= Begin %d, End %d"), Begin, End);
				Loop = false;
				continue;
			}
	...
	FTimespan EndToBeginTimespan = TransformWithTime[End].DateTime - TransformWithTime[Begin].DateTime;
	FTimespan EndToClientTimespan = SyncDateTime - TransformWithTime[Begin].DateTime;

	double EndToBegin_ms = EndToBeginTimespan.GetTotalMilliseconds();
	double EndToClient_ms = EndToClientTimespan.GetTotalMilliseconds();

	float Alpha = EndToClient_ms / EndToBegin_ms;
	UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Begin Time %lld, End Time %lld"), TransformWithTime[Begin].DateTime.GetTicks(), TransformWithTime[End].DateTime.GetTicks());
	//UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Alpha %f"), Alpha);

	FVector Lerp_LocationData = FMath::Lerp(TransformWithTime[Begin].Location, TransformWithTime[End].Location, Alpha);
	FVector Lerp_RotationData = FMath::Lerp(TransformWithTime[Begin].Rotation, TransformWithTime[End].Rotation, Alpha);

	FTransformWithTime OutDate;
	OutDate.Location = Lerp_LocationData;
	OutDate.Rotation = Lerp_RotationData;
	OutDate.Initialized = true;
	return OutDate;
}
#endif


AActor* AShooterCharacter::GetOrSpawn_DebugBlueActor()
{
	if (BlueActorInstance == nullptr)
	{
		BlueActorInstance = GetWorld()->SpawnActor<AActor>(BlueActor);
	}

	return BlueActorInstance;
}


AActor* AShooterCharacter::GetOrSpawn_DebugRedActor()
{
	if (RedActorInstance == nullptr)
	{
		RedActorInstance = GetWorld()->SpawnActor<AActor>(RedActor);
	}
...
void AShooterCharacter::Destroyed()
{
	Super::Destroyed();
#if UE_SERVER
//	GetWorldTimerManager().ClearTimer(UpdateTTransformWithTimerHandle);
#endif

	if (BlueActorInstance != nullptr) {
		BlueActorInstance->Destroy();
	}

	if (RedActorInstance != nullptr) {
		RedActorInstance->Destroy();
	}
...
void AShooterCharacter::SetActorBonesTransform(TArray<FTransform> transform) {
	auto test_mesh = GetPawnMesh();
	//test_mesh->
}
...
#if UE_SERVER
	FDateTime Now = FDateTime::Now();
	FTimespan Timespan = Now - CharacterTransformWithTimeTimer;
	if (Timespan.GetTotalMilliseconds() > 100.0f)
	{
		UpdateTransformWithTimer();
		CharacterTransformWithTimeTimer = Now;
	}
/*
	AActor* DebugRedActor = GetOrSpawn_DebugBlueActor();
	if (DebugRedActor == nullptr)
	{
		//UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Get Red actor Failed"));
		return;
	}

	DebugRedActor->SetActorLocation(GetActorLocation());
*/
#endif
	if (a == 0) {
		...
		auto animation_script_instance = GetMesh()->AnimScriptInstance;
		if (animation_script_instance == nullptr) {
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Tick Not has animation script instance"));
			return;
		}
		else {
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Tick Has animation script instance"));
		}

		
		auto montage_instance = animation_script_instance->GetCurrentActiveMontage();
		if (montage_instance != nullptr) {
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Tick Montage Instance"));
		}
		else {
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Tick Not has Montage Instance"));
		}
		++a;
	}
	...
	if (b == 3000) {
		auto test_mesh = GetPawnMesh();
		auto bones_transform = test_mesh->GetBoneSpaceTransforms();
		auto copy_actor = GetWorld()->SpawnActor<AShooterCharacter>(StaticClass(),GetActorLocation(), GetActorRotation());
		//copy_actor->
