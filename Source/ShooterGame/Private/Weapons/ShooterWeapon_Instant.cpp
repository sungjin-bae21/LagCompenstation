// Copyright Epic Games, Inc. All 0Rights Reserved.
...
bool AShooterWeapon_Instant::ServerNotifyHit_Validate(const FHitResult& Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread, FDateTime ShootTime, FVector ShootPosition)
{
	return true;
}

void AShooterWeapon_Instant::ServerNotifyHit_Implementation(const FHitResult& Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread, FDateTime ShootTime, FVector ShootPosition)
{
#if UE_SERVER
	// 클라이언트에서 Hit event 발생 이벤트가 발생한 시점과 Character Position 을 같이 전송한다.
	// 
	// 전송해야 될 내용. 
	//		CharacterPosition, DateTime
	// 
	// 	시간을 받아서 계산을 한다.
	...
	if (GetInstigator() && (Impact.GetActor() || Impact.bBlockingHit))
	{
		...
		{	
			// 지연 보상 계산.
			// (Hit 이벤트가 들어온 시점 - RTT) 시간을 기준으로 Transform 정보를 가지고 온다.
			float RTT = MyPawn->GetPlayerRTT();
			 FDateTime CompensationDateTime = ShootTime - FTimespan::FromMilliseconds(RTT);
			AShooterCharacter* Target = Cast<AShooterCharacter>(Impact.GetActor());
			if (Target == nullptr) 
			{
				//UE_LOG(LogTemp, Error, TEXT("[MyTestLog] Target character is null"));
				return;
			}

			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] ShootTime %lld"), ShootTime.GetTicks());
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Now %lld"), FDateTime::Now().GetTicks());
			UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] MyPawn Rtt %f"), RTT);

			FTransformWithTime TransfromWithTime = Target->GetTransformAtTime(CompensationDateTime, MyPawn->ClientSyncTimespan);

			AActor* DebugRedActor = Target->GetOrSpawn_DebugRedActor();
			...
			DebugRedActor->SetActorLocation(TransfromWithTime.Location);


			TransfromWithTime = Target->GetTransformAtTime(ShootTime, MyPawn->ClientSyncTimespan);
			AActor* DebugBlueActor = Target->GetOrSpawn_DebugBlueActor();
			if (DebugBlueActor == nullptr)
			{
				//UE_LOG(LogTemp, Warning, TEXT("[MyTestLog] Get Blue actor Failed"));
				return;
			}
			else

			DebugBlueActor->SetActorLocation(TransfromWithTime.Location);
...
void AShooterWeapon_Instant::ProcessInstantHit(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
	if (MyPawn && MyPawn->IsLocallyControlled() && GetNetMode() == NM_Client)
	{
		...
		if (Impact.GetActor() && Impact.GetActor()->GetRemoteRole() == ROLE_Authority)
		{
			...
			ServerNotifyHit(Impact, ShootDir, RandomSeed, ReticleSpread, FDateTime::Now(), GetActorLocation());
		}
