...
// deidcated server
#if UE_SERVER
	TArray<FTransformWithTime> TransformWithTime;
	//FTimerHandle UpdateTTransformWithTimerHandle;
#endif
	FTimerHandle SyncTimeWithServerHandle;
...
public:
	...
	AActor* GetOrSpawn_DebugBlueActor();
	AActor* GetOrSpawn_DebugRedActor();

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DebugObject)
	TSubclassOf<AActor>  BlueActor;
	AActor* BlueActorInstance = nullptr;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DebugObject)
	TSubclassOf<AActor>  RedActor;
	AActor* RedActorInstance = nullptr;
	...
	UPROPERTY(BlueprintReadOnly)
	FPoseSnapshot MySnapshot;

	UPROPERTY(BlueprintReadOnly)
	bool ChangeSnapShot;

	UFUNCTION(BlueprintCallable, Category = "Update")
	void SetBoneTransform(FPoseSnapshot SnapShot_);

	UFUNCTION(BlueprintCallable, Category = "Update")
	void UpdatePreviousTransform();

	void SyncTimerWithServer();
// dedicated server
#if UE_SERVER
	void UpdateTransformWithTimer();
	FTransformWithTime GetTransformAtTime(FDateTime DateTime, FTimespan Other_ClientSyncTimespan);

	FDateTime CharacterTransformWithTimeTimer;
#endif

	
	float GetPlayerRTT();
	void SetActorBonesTransform(TArray<FTransform> transform);

	UFUNCTION(reliable, server, WithValidation)
    void TimeSync(FDateTime DateTime);

	FTimespan ClientSyncTimespan;
